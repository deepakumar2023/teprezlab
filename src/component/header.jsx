...
export default function Header() {
      ...
      <div className=" mx-auto flex items-center justify-between px-4 py-3">
        ...
        <Link href="/" className="flex items-center">
          <div className="flex items-center">
            <img src="/TEP_logo5.png" alt="Logo" className="h-15 w-auto" />
          </div>
        </Link>
        ...
        <nav className="hidden md:flex space-x-8">
          {menuItems.map((item) => {
            const isActive = isShopPage && item.link === "/shop";
            ...
            return (
              <Link
                ...
                className={`font-semibold transition ${
                  isShopPage
                    ? isActive
                      ? "text-blue-500" // ✅ only shop blue
                      : "text-black" // ❌ no hover on others
                    : scrolled
  ...
  className={`md:hidden text-2xl ${
    isShopPage
      ? "text-black"            // ✅ always black on /shop
      : scrolled
      ? "text-black"
      : "text-white"
  }`}
